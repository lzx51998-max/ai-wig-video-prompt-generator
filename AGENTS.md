...
13. 把图片观察、用户要求和覆盖项写入 `.rag-workbench/request.json`。普通模式运行 `python -m rag_app prepare --request .rag-workbench/request.json --subject-image <人物图路径> --background-image <背景图路径> --output .rag-workbench/prepared.json`；遮挡变装模式运行 `python -m rag_app prepare --request .rag-workbench/request.json --before-subject-image <转场前人物图路径> --after-subject-image <转场后人物图路径> --background-image <背景图路径> --output .rag-workbench/prepared.json`。程序用图片内容哈希判断参考图是否更换；无阻塞项后再写入 `.rag-workbench/draft.json`，运行：
